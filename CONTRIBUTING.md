...
Veja abaixo as várias formas de contribuir conosco :smile:
...
```sh
git clone https://github.com/<seu_usuario>/Frontend-SoLanches.git
```

Onde seu usuário ficará no campo **<seu_usuario>**.

Ao terminar de clonar, será criado um repositório *Frontend-SoLanches* no seu computador. Agora você tem a liberdade de com seu editor favorito fazer as edições que você achar necessário, ao terminar, você deve commitar suas alterações no seu repositório remoto.

Entre no repositório do *Frontend-SoLanches*

```sh
cd Frontend-SoLanches/
```

...
import { deformatName, formatRoute } from '../../util/format'
  ...
  return (
    <Link to={`/${formatRoute(props.title)}`}>
      <div className={style.container}>
        ...
        <p className={style.title}>{deformatName(props.title)}</p>
        <hr className={style.line}></hr>
        <div className={style.containerInfos}>
          <span className={style.location}>
            <Location size='1.2rem' color='#FF983B' />
          </span>
          <p className={style.adress}>{props.adress}</p>
        </div>
        <div className={style.containerInfos}>
          <span className={style.infos}>
            <Time size='1.15rem' color='#FF983B' />
          </span>
          <div>
